...
- **Encrypted `.env.development` / `.env.production` files.** Decrypt with `pnpm dotenvx decrypt -f .env.development` (requires `DOTENV_PRIVATE_KEY_DEVELOPMENT` env var). Then create a plain `.env` with local overrides (DB paths, protocol). Run with `node --env-file=.env` so overrides take precedence over decrypted values.
